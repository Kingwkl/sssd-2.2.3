                ...
                const char *confdb_location)
{
     ...
     * events. 因为confdb调用使用sync ldb调用，所以我们在这里创建一个单独的事件上下文。 这将防止ldb sync调用启动嵌套事件。
                        *注意：这意味着我们*不能*进行异步调用并返回confdb，除非我们转换所有调用并挂接到主事件上下文。
    ...
    ret = ldb_connect(cdb->ldb, confdb_location, 0, NULL);      //连接到一个ldb后端
    umask(old_umask);
    if (ret != LDB_SUCCESS) {
