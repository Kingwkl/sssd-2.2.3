                     ...
                     struct confdb_ctx **_cdb)
{
    ...
    ret = confdb_init(tmp_ctx, &cdb, cdb_file);   //配置文件加入到cdb数据库中
    if (ret != EOK) {
    ...
    /* Initialize the CDB from the configuration file 初始化cdb*/
    ret = confdb_test(cdb);
    if (ret == ENOENT) {
        /* First-time setup */

        /* Purge any existing confdb in case an old
         * misconfiguration gets in the way       清除任何现有的confdb以防旧的配置错误
         */
        talloc_zfree(cdb);
        ret = unlink(cdb_file);
