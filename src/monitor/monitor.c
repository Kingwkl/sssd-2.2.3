                           ...
                           struct mt_ctx **monitor)
{
    ...
    /* Read in the monitor's configuration 验证数据库中的配置读取监视器的配置*/
...
//poptOption结构体，前两个参数，long name和short name，定义了选项的名字，前者是长名字，后者是单一字符
//第三个参数argInfo指定了popt选项后面期待的参数类型，若没有参数，POPT_ARG_NONE将被使用
//POPT_ARG_STRING 无需执行类型检查，参数类型是char * , 选项参数的类型可接受任意的字符串
//第四个参数是val，是popt语法分析函数在遇到该选项时将要返回的值，若为0,则不返回，而继续分析下一个命令行参数
//第五、六个蚕食是descrip和argDescrip，前者是关于参数的文本描述，后者是关于选项参数的类型的简短概要；为NULL即为不需要
    struct poptOption long_options[] = {     //应用程序通过“选项表”的方式为popt提供命令行选项信息
        POPT_AUTOHELP
        SSSD_MAIN_OPTS
        SSSD_LOGGER_OPTS
    ...
    pc = poptGetContext(argv[0], argc, argv, long_options, 0);              //创建一个popt上下文
    while((opt = poptGetNextOpt(pc)) != -1) {                               //获取参数，当最后一个参数分析完成，poptGetNextOpt()返回-1，当出现错误时，返回其他的负值
        switch(opt) {
        default:
            fprintf(stderr, "\nInvalid option %s: %s\n\n",
                    poptBadOption(pc, 0), poptStrerror(opt));
            poptPrintUsage(pc, stderr, 0);                      //poptPrintUsage() 则打印简短的用法信息
            return 1;
        }
    }
     ...
     * to save it and pass it to the children later.  如果级别或时间戳是在命令行中传递的，我们希望将其保存并稍后传递给子级。
     */
    ...
    tmp_ctx = talloc_new(NULL);   //创建新的临时工作环境
    if (!tmp_ctx) {
        return 7;
    }
    ...
    if (opt_config_file) {
        ...
        config_file = talloc_strdup(tmp_ctx, SSSD_CONFIG_FILE);  //将前者copy至后者
    }
...
#ifdef USE_KEYRING
    /* Do this before all the forks, it sets the session key ring so all            在所有fork之前执行此操作，它会设置会话密钥环，
     * keys are private to the daemon and cannot be read by any other process       以便所有密钥都是守护程序专用的，并且任何其他进程树都无法读取
     * tree */ 
    ...
    ret = keyctl_setperm(KEY_SPEC_SESSION_KEYRING, KEY_POS_ALL);   //更改密钥权限的掩码，赋予进程全部权限
    if (ret == -1) {
        sss_log(SSS_LOG_ALERT,
    ...
    /* Check if the SSSD is already running and for nscd conflicts unless we're  除非我们只对重新读取配置感兴趣，否则请检查SSSD是否已在运行以及是否存在nscd冲突。
